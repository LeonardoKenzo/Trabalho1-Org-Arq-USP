...
add_start:
	add s2, zero, s0 #usa o registrador s2 como endere√ßo do primeiro vagao
...
list_train:
	add t1, zero, s0 # t1 comeca apontando para o primeiro vag√£o
...
list_loop:
    	...
    	addi a0, zero, 10       # ASCII 10 √© o \n
    	addi a7, zero, 11
    	ecall
    
    	lw t1, 8(t1) # recebe o endere√ßo do proximo vagao (offset 8)
    	j list_loop
...
search_wagon:
	...
	add t2, zero, s0 # ComeÁar do primeiro vagao
	
search_loop:
	beq t2, zero, search_not_found # Pular para caso erro
	
	lw t3, 0(t2) # t3 recebe o ID do vagao atual
	beq t1, t3, search_found # Se o ID em t1 e t3 È igual, achamos
